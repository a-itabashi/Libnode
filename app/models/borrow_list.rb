class BorrowList < ApplicationRecord
  ...
  scope :return_date_of_borrowing_books, ->(user) { where(user: user.id).where(is_return: false).pluck(:return_date) }
  scope :book_id_of_borrowing_books, ->(user) { where(user: user.id).where(is_return: false).pluck(:book_id) }
